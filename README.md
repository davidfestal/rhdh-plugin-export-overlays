...
- **Release branches (`release-x.y`)**: These are long-running branches, each corresponding to a specific RHDH release (e.g., `release-1.6`).
  ...
  - After creation, they only receive pull requests for updates to existing plugins. No new workspaces can be added directly to a release branch.
...
To add this additional configuration (excluding the patches, since the patch filees are placed and appliedat the workspace [monorepo] level):
